...
object p11 {
  ...
  def pairToProduct(pair : (Int, Int), tmpl : List[(Int, Int)]) = {
    tmpl.map({xy => pairPlus(pair, xy)}).map({xy => data(xy._1)(xy._2)}).product
  }
  ...
  val one   = List((0,0), (0,1), (0,2), (0,3))
  val two   = List((0,0), (1,0), (2,0), (3,0))
  val three = List((0,0), (1,1), (2,2), (3,3))
  val four  = List((3,0), (2,1), (1,2), (0,3))

  val ones = for(
    x <- List.range(0, 20);
    y <- List.range(0, 20-4)) yield pairToProduct((x,y), one)
